...
class GPT2PPL:
    ...
    def __call__(self, sentence):
        ...
        lines = self._preprocess(sentence)

        results["perplexity"] = self._getPerplexity(sentence)
        perplexity_per_line = self._getPPL(lines)

        results["avg_ppl"] = sum(perplexity_per_line)/len(perplexity_per_line) # average perplexity per line
        results["burstiness"] = max(perplexity_per_line)

        verdict, label = self._getResults(results["avg_ppl"])
        results["label"] = label
    ...
    def _getResults(self, threshold):
        if threshold < 60:
            label = 0
    ...
    def _getPPL(self, lines):
        ...
        for line in lines:
            ...
            if line[-1] == "\n" or line[-1] == " ":
                ...
                offset = line[-1]
                line = line[:-1]
            ppl = self._getPerplexity(line)
            perplexity_per_line.append(ppl)

        return perplexity_per_line


    def _getPerplexity(self,sentence):
    ...
    def _preprocess(self, sentence):
        ...
        assert total_valid_char > 100, "Please input more text (min 100 characters)"
    ...
    def getScore(self, sentence):
        results = self(sentence)
        avg_ppl = results["avg_ppl"]

        return 1 - (avg_ppl / 100)
